...
test.describe("Search Bug", () => {
  ...
  test("Search bugs by title on the board page", async ({ boardPage }) => {
    ...
    await expect(
      await boardPage.getBugCellByTitle("Login page crashes on empty password")
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle("Login page redirect fails after auth")
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle("Login page error message not shown")
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle("Login page enter key stops working")
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle("Board page fails to load")
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle(
        "Search field disappears on page resize"
      )
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle(
        "Severity dropdown missing on create page"
      )
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle(
        "Create page modal does not close on cancel"
      )
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle("Title bar logo missing on page load")
    ).toBeVisible();
    await expect(
      await boardPage.getBugCellByTitle("Sort resets on page refresh")
    ).toBeVisible();
