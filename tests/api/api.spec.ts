...
test.describe("BuggyBoard REST API", () => {
  ...
  test("POST /api/login fails when missing credentials", async ({
    request,
  }) => {
  ...
  test("POST /api/login fails with invalid credentials", async ({
    request,
  }) => {
    const response = await request.post(`${apiBase}/api/login`, {
  ...
  test("GET /api/bugs returns array of bugs", async ({ request }) => {
    ...
    try {
      ...
      expect(list).toEqual(
        expect.arrayContaining([expect.objectContaining({ id: createdBug.id })])
      );
    } finally {
  ...
  test("POST /api/bugs creates a bug and returns normalized values", async ({
    request,
  }) => {
  ...
  test("POST /api/bugs fails when severity is invalid", async ({ request }) => {
    const response = await request.post(`${apiBase}/api/bugs`, {
      data: {
        title: uniqueValue("invalid severity"),
        severity: "critical",
        owner: "API",
        description: "desc",
      },
  ...
  test("POST /api/bugs fails when description is blank", async ({
    request,
  }) => {
    const response = await request.post(`${apiBase}/api/bugs`, {
      data: {
        title: uniqueValue("blank description"),
        severity: "low",
        owner: "API",
        description: "",
      },
  ...
  test("GET /api/bugs/:id returns 404 when bug is not found", async ({
    request,
  }) => {
  ...
  test("PUT /api/bugs/:id returns 400 for invalid bug id", async ({
    request,
  }) => {
  ...
  test("PUT /api/bugs/:id returns 404 when bug not found", async ({
    request,
  }) => {
  ...
  test("PUT /api/bugs/:id fails with invalid state", async ({ request }) => {
    const { body } = await createBug(request, {
      title: uniqueValue("invalid state"),
    });
    const id = body.id;
    try {
  ...
  test("PUT /api/bugs/:id fails with blank fields", async ({ request }) => {
    const { body } = await createBug(request, {
      title: uniqueValue("blank-fields"),
    });
    const id = body.id;
    try {
      const response = await request.put(`${apiBase}/api/bugs/${id}`, {
  ...
  test("DELETE /api/bugs/:id removes bug and returns 204", async ({
    request,
  }) => {
    const { body } = await createBug(request, {
      title: uniqueValue("DELETE bug"),
    });
    const id = body.id;
  ...
  test("DELETE /api/bugs/:id returns 400 for invalid id", async ({
    request,
  }) => {
  ...
  test("DELETE /api/bugs/:id returns 404 when bug does not exist", async ({
    request,
  }) => {
